...
#           2014-2015 Vimal Manohar (Johns Hopkins University)
# Apache 2.0
#
...
#
# This script differs from get_egs2.sh because this uses nnet-get-weighted-egs
# instead of nnet-get-egs. Currently this supports frame selection, where
# we select a frame only if its frame-level confidence score is above a 
# threshold. Additionally, only frames_per_eg=1 is supported.
#
...
weight_threshold=0.7      # Only select frames above this score
...
if [ $# != 3 ]; then
  echo "Usage: $0 [opts] <data> <ali-dir|best-path-dir> <egs-dir>"
...
for f in $data/feats.scp $alidir/ali.1.gz $alidir/weights.1.gz $alidir/final.mdl $alidir/tree $extra_files; do
...
if [ $stage -le 2 ]; then
  ...
  for id in $(seq $nj); do gunzip -c $alidir/weights.$id.gz; done | \
    copy-vector ark:- ark,t:- | \
    utils/filter_scp.pl <(cat $dir/valid_uttlist $dir/train_subset_uttlist) | \
    gzip -c >$dir/weights_special.gz || exit 1;
  ...
  $cmd $dir/log/create_valid_subset.log \
    ...
    --weight-threshold=$weight_threshold "$valid_feats" \
    "ark,s,cs:gunzip -c $dir/ali_special.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" \
    "ark,s,cs:gunzip -c $dir/weights_special.gz |" \
    "ark:$dir/valid_all.egs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset.log \
    nnet-get-weighted-egs $ivectors_opt $nnet_context_opts \
    --use-frame-selection=$use_frame_selection \
    --use-frame-weights=$use_frame_weights \
    --weight-threshold=$weight_threshold "$train_subset_feats" \
    "ark,s,cs:gunzip -c $dir/ali_special.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" \
    "ark,s,cs:gunzip -c $dir/weights_special.gz |" \
...
if [ $stage -le 3 ]; then
  ...
  if [ ! -z $postdir ]; then
    $cmd $io_opts JOB=1:$nj $dir/log/get_egs.JOB.log \
      nnet-get-weighted-egs $ivectors_opt $nnet_context_opts \
      --num-frames=$frames_per_eg \
      --use-frame-selection=$use_frame_selection \
      --use-frame-weights=$use_frame_weights \
      --weight-threshold=$weight_threshold "$feats" \
      scp:$postdir/post.JOB.scp "ark:gunzip -c $alidir/weights.JOB.gz |" ark:- \| \
    ...
    $cmd $io_opts JOB=1:$nj $dir/log/get_egs.JOB.log \
      ...
      --use-frame-weights=$use_frame_weights \
      --weight-threshold=$weight_threshold "$feats" \
      "ark,s,cs:gunzip -c $alidir/ali.JOB.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" "ark:gunzip -c $alidir/weights.JOB.gz |" ark:- \| \
      nnet-copy-egs ark:- $egs_list || exit 1;
  fi

  #log_files=
  #for n in `seq $nj`; do 
  #  log_files="$log_files $dir/log/get_egs.$n.log"
  #done
  #num_frames=$(grep "Read .* neural-network training examples, wrote .*" $log_files | perl -pe 's/.+, wrote (\S+).*/$1/' | awk '{i+=$1} END{print i}')
  #echo $num_frames > $dir/info/num_frames
fi
...
#num_archives=$[$num_frames/($frames_per_eg*$samples_per_iter)+1]
#
## (for small data)- while reduce_frames_per_eg == true and the number of # archives is 1 and would still be 1 if we reduced frames_per_eg by 1, reduce it
## by 1.
#reduced=false
#while $reduce_frames_per_eg && [ $frames_per_eg -gt 1 ] && \
#  [ $[$num_frames/(($frames_per_eg-1)*$samples_per_iter)] -eq 0 ]; do
