...
if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! $mfccdir/storage ]; then                                                                                                          
  utils/create_split_dir.pl /export/b0{1,2,3,4}/$USER/kaldi-data/fisher_english-$(date '%m_%d_%h_%M')/s5/mfcc/storage $mfccdir/storage
fi
