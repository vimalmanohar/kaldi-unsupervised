...
if [ $stage -le 0 ] ; then
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.LMWT.log \
    ...
    paste -d ' ' \<\(cut -f 1-4 -d ' ' $dir/score_LMWT/${name}.ctm.sorted \) \
        ...
        \> $dir/score_LMWT/${name}.ctm.tmp '&&' \
    utils/fix_ctm.sh $dir/score_LMWT/stm $dir/score_LMWT/${name}.ctm.tmp '&&' \
    $SortingProgram sortCTM \<$dir/score_LMWT/${name}.ctm.tmp \> $dir/score_LMWT/${name}.ctm '&&' \
