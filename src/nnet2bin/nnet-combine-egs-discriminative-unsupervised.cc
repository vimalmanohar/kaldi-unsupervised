// nnet2bin/nnet-combine-egs-discriminative-unsupervised.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Copy examples for discriminative unsupervised neural network training,\n"
        ...
        "\n"
        "Usage:  nnet-combine-egs-discriminative-unsupervised [options] <egs-rspecifier> <egs-wspecifier>\n"
        "\n"
        "e.g.\n"
        "nnet-combine-egs-discriminative-unsupervised --max-length=512 ark:temp.1.degs ark:1.degs\n";
    ...
    SequentialDiscriminativeUnsupervisedNnetExampleReader example_reader(
        examples_rspecifier);
    DiscriminativeUnsupervisedNnetExampleWriter example_writer(
        examples_wspecifier);

    int64 num_read = 0, num_written = 0, num_discarded = 0;

    while (!example_reader.Done()) {
      std::vector<DiscriminativeUnsupervisedNnetExample> buffer;
      ...
      std::vector<DiscriminativeUnsupervisedNnetExample> combined;
      CombineDiscriminativeUnsupervisedExamples(max_length, buffer, &combined);
      buffer.clear();
      for (size_t i = 0; i < combined.size(); i++) {
        const DiscriminativeUnsupervisedNnetExample &eg = combined[i];
