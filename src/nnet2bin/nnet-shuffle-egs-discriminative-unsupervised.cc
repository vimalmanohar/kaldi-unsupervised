// nnet2bin/nnet-shuffle-egs-discriminative-unsupervised.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Copy examples (typically single frames) for unsupervised \n"
        "neural network training with NCE criterion,\n"
        ...
        "\n"
        "Usage:  nnet-shuffle-egs-discriminative-unsupervised [options] <egs-rspecifier> <egs-wspecifier>\n"
        "\n"
        "nnet-shuffle-egs-discriminative-unsupervised --srand=1 ark:train.degs ark:shuffled.degs\n";
    ...
    if (buffer_size == 0) { // Do full randomization
      ...
      for (; !example_reader.Done(); example_reader.Next())
        egs.push_back(new DiscriminativeUnsupervisedNnetExample(
      ...
      for (; !example_reader.Done(); example_reader.Next()) {
        ...
        if (egs[index] == NULL) {
          egs[index] = new DiscriminativeUnsupervisedNnetExample(example_reader.Value());
        } else {
