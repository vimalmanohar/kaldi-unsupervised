// nnet2bin/nnet-train-discriminative-unsupervised-smoothed.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "function (NCE) in an unsupervised way. This is similar to \n"
        "nnet-train-discriminative-unsupervised-simple, but does an update \n"
        "only after looking at all the examples.\n"
        "\n"
        "Usage:  nnet-train-discriminative-unsupervised-smoothed [options] <model-in> <training-examples-in> <model-out>\n"
        "e.g.:\n"
        "nnet-train-discriminative-unsupervised-smoothed 1.nnet ark:1.uegs 2.nnet\n";
    ...
    TransitionModel trans_model;
    {
      ...
      Nnet nnet_gradient(am_nnet.GetNnet());
      ...
      for (; !example_reader.Done(); example_reader.Next(), num_examples++) {
        ...
        KALDI_ASSERT(objf1.ApproxEqual(objf2));
        ...
        
      {
        ...
        nnet_gradient.Write(ko.Stream(), binary_write);
