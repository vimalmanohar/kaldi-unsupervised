...
static void ProcessFile(const MatrixBase<BaseFloat> &feats,
  ...
  if (num_frames != 1) {
    KALDI_ERR << "Currently this program supports only num-frames=1";
  }

  if (use_frame_weights) {
    KALDI_ERR << "use-frame-weights was set to true. "
              << "Frame weights are currently supported. "
              << "Try use-frame-selection instead.";
  }

  for (int32 t = 0; t < feats.NumRows(); t += num_frames) {
    if (use_frame_selection) {
      if (weights(t) < weight_threshold) {
        (*num_frames_skipped)++;
        continue;
      }
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("weight-threshold", &weight_threshold, 
                "Keep only frames with weights above this threshold.");
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!pdf_post_reader.HasKey(key)) {
        ...
        if (!weights_reader.HasKey(key)) {
          ...
          ProcessFile(feats, pdf_post, key, weights,
                      left_context, right_context, num_frames,
                      const_feat_dim, weight_threshold,
    ...
    KALDI_LOG << "Finished generating examples, "
              ...
              << " skipped " << num_frames_skipped << " frames; "
